...
class PoemSearchEngine {
    ...
    /**
     * 初始化：解析紧凑数据并构建倒排索引
     * @param {Object} dict - POEM_DICT: {a: authors[], t: titles[]}
     * @param {Object} five - POEM_FIVE: {t: text, a: authorCSV, i: titleCSV}
     * @param {Object} seven - POEM_SEVEN: same format
     */
    init(dict, five, seven) {
        ...
        this.isReady = true;
        console.log(`搜索引擎已初始化，共 ${this.fiveCount + this.sevenCount} 条记录`);
    }
    ...
    search(options) {
        ...
        const searchFive = options.type !== 'seven';
        const searchSeven = options.type !== 'five';
     ...
     */
    getRandom(type) {
        if (type === 'five' || (type === 'all' && Math.random() < this.fiveCount / (this.fiveCount + this.sevenCount))) {
...
const poemSearchEngine = new PoemSearchEngine();
